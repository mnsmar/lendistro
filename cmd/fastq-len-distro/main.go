...
const descr = "Measure the length distribution of the records in FASTQ file/s. " +
	cmd.OutDescription
...
func (Opts) Version() string { return "fastq-len-distro 0.1" }

// Description returns an extended description of the program.
func (Opts) Description() string { return descr }

// reader encapsulates a fastq.Reader and satisfies lendistro.Reader.
type reader struct{ r *fastq.Reader }
...
func main() {
	...
	for _, input := range opts.Input {
		...
		go func(input string) {
			...
			// create BED reader.
			r, err := fastq.NewReader(f, linear.NewSeq("", nil, alphabet.DNA)), nil
			if err != nil {
				log.Fatal(err)
			}
